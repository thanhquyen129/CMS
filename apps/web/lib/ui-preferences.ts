...
export function uiPreferencesBootScript(): string {
  ...
  return `(function(){try{var d=${def},p=null;try{p=localStorage.getItem(${JSON.stringify(storage)});}catch(e){}if(!p){var m=document.cookie.match(/(?:^|; )${cookie}=([^;]*)/);if(m)p=decodeURIComponent(m[1]);}if(p){try{var o=JSON.parse(p);for(var k in o){if(Object.prototype.hasOwnProperty.call(d,k)&&k!=="theme")d[k]=o[k];}}catch(e){}}d.theme="lcms";var r=document.documentElement;r.setAttribute("data-theme","lcms");r.setAttribute("data-layout",d.layout);r.setAttribute("data-density",d.density);r.setAttribute("data-zebra",d.tableZebra?"1":"0");r.setAttribute("data-sticky-nav",d.stickyNav?"1":"0");r.setAttribute("data-reduce-motion",d.reduceMotion?"1":"0");r.setAttribute("data-nav-labels",d.showNavLabels?"1":"0");r.setAttribute("data-show-queues",d.showQueues?"1":"0");var sm=document.cookie.match(/(?:^|; )lcms_sidebar=([^;]*)/);r.setAttribute("data-sidebar",sm&&sm[1]==="collapsed"?"collapsed":"expanded");}catch(e){}})();`;
}
