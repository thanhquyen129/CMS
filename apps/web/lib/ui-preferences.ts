...
export function uiPreferencesBootScript(): string {
  ...
  return `(function(){try{var d=${def},p=null;try{p=localStorage.getItem(${JSON.stringify(storage)});}catch(e){}if(!p){var m=document.cookie.match(/(?:^|; )${cookie}=([^;]*)/);if(m)p=decodeURIComponent(m[1]);}if(p){try{var o=JSON.parse(p);if(o.theme)d.theme=o.theme;if(o.layout)d.layout=o.layout;if(o.density)d.density=o.density;}catch(e){}}var r=document.documentElement;r.setAttribute("data-theme",d.theme);r.setAttribute("data-layout",d.layout);r.setAttribute("data-density",d.density);}catch(e){}})();`;
}
