...
| UX-01 / UI-09 | Thanh toán & Thu tiền — `/settlements*` | Payment/Collection + allocate; so PNG UI-09 | Cash ≠ settled; allocate mới giảm outstanding | **Pass** | Tabs Thanh toán/Thu tiền; copy phân bổ mới giảm outstanding |
...
| UX-01 / W2-close-gates | Gate chốt — `/financial-closes/[id]` | Mở phiên chưa đủ điều kiện; thử chốt | Checklist gate pass/fail rõ; nút chốt bị chặn khi gate fail | **Pass** | 5 gate Chặn (ngoại lệ/chứng từ/AP-AR/phân bổ/tiền chưa gán) |
...
| UX-04 | 2 user: Cost-only vs Revenue-only | Đăng nhập lần lượt; mở dashboard, aging, menu | Menu/action theo permission + data scope; backend 403 khi vượt quyền | **Pass*** | Live `cost@` post-deploy `cb7f1b7`: dashboard ghi chú ẩn DT/biên; `/revenues` + `/api/profitability/bills` → 403 «không có quyền xem doanh thu»; `/api/costs` 200. *Residual:* nav/CTA Doanh thu vẫn hiện (license UI ≠ authz) |
