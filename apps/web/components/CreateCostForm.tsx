...
export function CreateCostForm({
  ...
  return (
    <form className="receive-form" onSubmit={onSubmit} noValidate>
      <p className="note">
        Tạo {costLabel.toLowerCase()} <strong>trực tiếp</strong> gắn {billLabel}, bắt đầu ở
        lớp {expected}. {costLabel} ≠ {paymentLabel}. Xác nhận / ghi nhận thực tế
        trên hồ sơ Bill sau khi tạo.
      </p>
      ...
      <div className="cta-row">
        <button type="submit" className="btn" disabled={busy}>
          {busy ? "Đang tạo…" : `Tạo ${costLabel.toLowerCase()}`}
        </button>
      </div>
    </form>
  );
