  ...
  return (
    <>
      <fieldset className="group-box">
        <legend>Ngữ cảnh so sánh</legend>
        <form className="form-grid" onSubmit={onSubmit}>
          ...
          <div className="field">
            <label htmlFor="quantity">Trọng lượng tính cước (kg)</label>
        ...
        <>
          <div className="stat-grid stat-grid-3">
            {(() => {
              ...
              const bestTotal = ok.length
                ? Math.min(...ok.map((q) => q.totalAmount))
                : null;
              return quotes.map((q) => {
                const isBest =
                  bestTotal != null && !q.error && q.totalAmount === bestTotal;
                return (
                  <article
                    key={q.rateCardId + q.versionNo}
                    className={`stat-card${isBest ? " stat-card-best" : ""}`}
                  >
                    {isBest ? (
                      <span className="badge badge-info">Tốt nhất</span>
                    ) : null}
                    <p>
                      {partyTypeLabel(q.partyType)} · {q.carrierName || q.cardName}
                    </p>
                    <p>
                      <code>{q.cardCode}</code> · v{q.versionNo}
                    </p>
                    <strong className="quote-price">
                      {q.error ? "—" : formatMoney(q.totalAmount, q.currencyCode)}
                    </strong>
                    {q.error ? (
                      <p className="alert alert-error">{q.error}</p>
                    ) : (
                      <p>
                        Base {formatMoney(q.baseAmount, q.currencyCode)} · Phụ phí{" "}
                        {formatMoney(q.surchargeAmount, q.currencyCode)}
                      </p>
                    )}
          ...
          <h3 className="section-title">Chi tiết so sánh</h3>
          <table className="data-table">
            <thead>
              <tr>
                <th>Tiêu chí</th>
                {quotes.map((q) => (
                  <th key={q.cardCode}>{q.carrierName || q.cardName}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                {quotes.map((q) => (
                  <td key={q.cardCode}>
                    {q.error ? q.error : formatMoney(q.totalAmount, q.currencyCode)}
                  </td>
                ))}
              </tr>
              <tr>
              ...
              <tr>
                <td>Rule</td>
                {quotes.map((q) => (
                  <td key={`${q.cardCode}-r`}>{q.ruleCodes || "—"}</td>
                ))}
              </tr>
            </tbody>
          </table>
          <p className="muted">
            So sánh không ghi Rating. Chọn bảng giá rồi tính trên Bill để lưu snapshot.
          </p>
        </>
      ) : null}
    </>
